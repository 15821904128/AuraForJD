...
SEX_LOAN_QUERY0="select cast(substr(t_loan.loan_time, 1, 10) as date) as sex_date, sum(t_loan.loan_amount) as total_loan_amount from t_loan join t_user on t_loan.uid = t_user.uid where t_user.sex = '01' group by cast(substr(t_loan.loan_time, 1, 10) as date)"
SEX_LOAN_QUERY1="select cast(substr(t_loan.loan_time, 1, 10) as date) as sex_date, sum(t_loan.loan_amount) as total_loan_amount from t_loan join t_user on t_loan.uid = t_user.uid where t_user.sex = '02' group by cast(substr(t_loan.loan_time, 1, 10) as date)"
